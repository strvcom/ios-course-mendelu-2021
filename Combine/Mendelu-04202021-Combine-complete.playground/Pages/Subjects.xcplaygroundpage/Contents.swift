...
class SubjectsViewController : UIViewController {
    ...
    var toggleBackgroundColor = PassthroughSubject<Void, Never>()
    var currentBackgroundColor = CurrentValueSubject<UIColor, Never>(.white)
    @Published var publishedBackgroundColor: UIColor? = .white
    ...
    override func loadView() {
        ...
        let view = UIView()
        view.backgroundColor = publishedBackgroundColor
    ...
    func binding() {
        toggleBackgroundColor
            .sink { [weak self] _ in
                self?.publishedBackgroundColor = self?.randomColor
            }
            .store(in: &cancellables)

        Publishers.Merge($publishedBackgroundColor, currentBackgroundColor.map { .some($0) })
            .assign(to: \.backgroundColor, on: self.view)
            .store(in: &cancellables)
    }

    @IBAction func changeColor() {
        toggleBackgroundColor.send(())
    }

    @IBAction func pickColor(colorPicker: UISegmentedControl) {
        currentBackgroundColor.value = colors[colorPicker.selectedSegmentIndex]
    }
}
